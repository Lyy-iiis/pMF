...
from utils.hsdp_util import get_hsdp_spec, MESH, AXIS_NAMES, REPLICATED_SHARDING, replicate
from utils.sample_util import create_global_sample_idx
...
def train_step(state: TrainState, batch, rng_init, ema_fn, lr_fn, aux_fn=None):
    ...
    """
    rng_base = random.fold_in(rng_init, state.step)
...
def sample_step(variable, sample_idx, model, rng_init, device_batch_size, 
    ...
    """
    rng_sample = jax.random.fold_in(rng_init, sample_idx[0])
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str) -> TrainState:
    ...
    state_sharding = jax.tree_map(
        lambda x: NamedSharding(MESH, get_hsdp_spec(x, AXIS_NAMES, 'hsdp')),
        state
    )

    expected_sharding = NamedSharding(MESH, P())

    def force_sharding_on_count(path, val):
        path_names = ''.join([str(p) for p in path])
        if 'ns_coeffs' in path_names or 'count' in path_names:
            local_devices = jax.local_devices()
            local_val = jax.device_get(val)
            local_shards = [jax.device_put(local_val, d) for d in local_devices]
    ...
    state = jax.tree_util.tree_map_with_path(force_sharding_on_count, state)

    batch_sharding = {
        "image": NamedSharding(MESH, P(AXIS_NAMES, None, None, None)),
        "label": NamedSharding(MESH, P(AXIS_NAMES,)),  # label shape [B,]
    }
...
def just_evaluate(config: ml_collections.ConfigDict, workdir: str) -> TrainState:
    ...
    for interval in config.sampling.interval:
        ...
        for omega in config.sampling.omegas:
            ...
            kwargs = replicate(kwargs)
            fid, is_score, best_ema = fid_evaluator(state, p_sample_step, step, ema_only=True, **kwargs)

            if fid < best_fid:
                best_fid, best_is, best_config = fid, is_score, (omega, t_min, t_max, best_ema)
